...
function BuildingHelper:InitGNV()
    local worldMin = Vector(GetWorldMinX()/2, GetWorldMinY()/2, 0)
    local worldMax = Vector(GetWorldMaxX()/2, GetWorldMaxY()/2, 0)
...
function BuildingHelper:SetupBuildingTable(abilityName, builderHandle)
    ...
    local pid = builderHandle:GetPlayerOwnerID()
    local unitName = _G.Player[pid].towerBuilding
    ...
    local fMaxScale = GetUnitKV(unitName, "ModelScale")
    if not fMaxScale then
...
function BuildingHelper:AddToQueue(builder, location, bQueued)
    ...
    if builder:GetUnitName() == buildingName then
        ...
        local model_location = Vector(location.x, location.y, location.z + model_offset - 48)

        -- npc_dota_creature doesn't render cosmetics on the particle ghost, use hero names instead
        local overrideGhost = GetUnitKV(buildingName, "OverrideBuildingGhost")
        ...
        local modelParticle = ParticleManager:CreateParticleForPlayer("particles/custom_effect/element_summon_effect.vpcf", PATTACH_CUSTOMORIGIN, nil, player)
        ParticleManager:SetParticleControl(modelParticle, 0, model_location)
        ParticleManager:SetParticleControl(modelParticle, 5, model_location)
        --ParticleManager:SetParticleControl(modelParticle, 1, model_location)
        ParticleManager:SetParticleControlEnt(modelParticle, 3, entity, 1, "attach_hitloc", entity:GetAbsOrigin(), true) -- Model attach          
...
function BuildingHelper:MeetsHeightCondition(location)
    if BuildingHelper.Settings["HEIGHT_RESTRICTION"] and BuildingHelper.Settings["HEIGHT_RESTRICTION"] ~= "" then
        return location.z >= BuildingHelper.Settings["HEIGHT_RESTRICTION"] and location.z<260
    else
        return true
    end
end
