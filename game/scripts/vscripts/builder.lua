...
function Build( event )
    ...
    local building_name = _G.Player[playerID].towerBuilding
    local gold_cost = _G.TowerInfo[building_name].cost
    ...
    event:OnPreConstruction(function(vPos)
        ...
        if  (_G.Player[playerID].eh_current+_G.TowerInfo[building_name].eh)>_G.Player[playerID].eh_limit  then
    ...
    event:OnBuildingPosChosen(function(vPos)
        ...
        _G.Player[playerID].eh_current=_G.Player[playerID].eh_current+_G.TowerInfo[building_name].eh
        -- Play a sound
    ...
    event:OnConstructionStarted(function(unit)
        ...
        -- Give item to cancel
        unit.ability_building_cancel = unit:AddAbility("CancelBuilding")
        if unit.ability_building_cancel then 
            unit.ability_building_cancel:SetLevel(1)
        ...
        unit:AddAbility("no_damage_incoming"):SetLevel(1)
    end)
    ...
    event:OnConstructionCompleted(function(unit)
        ...
        -- Remove the item
        if unit.ability_building_cancel then
            unit:RemoveAbility("CancelBuilding")
        end

        -- Give the unit their original attack capability
        unit:SetAttackCapability(unit.original_attack)
        Tower:InitUnit(unit,_G.TowerInfo[unit:GetUnitName()].cost)
    end)
...
function CancelBuilding( keys )
    ...
    if building.gold_cost then
        ...
        _G.Player[playerID].eh_current=_G.Player[playerID].eh_current-_G.TowerInfo[building:GetUnitName()].eh
    end
